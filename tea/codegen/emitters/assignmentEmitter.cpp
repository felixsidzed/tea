...
namespace tea {
	void CodeGen::emitAssignment(AssignmentNode* node) {
		...
		if (!ptr)
			TEA_PANIC("cannot assign to a value of type '%s'", type2readable(lhsType).data);

		if (rhsType.llvm != LLVMGetElementType(lhsType.llvm)) {
			TEA_PANIC("assignment type mismatch: cannot assign %s to %s. line %d, column %d",
				type2readable(rhsType).data, type2readable(LLVMGetElementType(lhsType.llvm)).data, node->line, node->column);
		}
